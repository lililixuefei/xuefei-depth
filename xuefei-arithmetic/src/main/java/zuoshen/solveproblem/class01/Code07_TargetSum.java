package zuoshen.solveproblem.class01;
...
public class Code07_TargetSum {
	...
	public static void main(String[] args) {
		int[] arr = {-1, -2};
		int target = 1;
//		int ways = findTargetSumWays1(arr, target);
		int ways = findTargetSumWays(arr, target);
		System.out.println("The number of ways to get " + target + " from " + arr + " is " + ways);
	}
	...
	public static int process_me(int[] arr, int index, int rest, int[][] dp) {
		if (dp[index][rest] != 0) {
		...
		if (index == arr.length) {
			...
			ans = process_me(arr, index + 1, rest - arr[index], dp) + process_me(arr, index + 1, rest + arr[index], dp);
		}
		dp[index][rest] = ans;
		return ans;
	}
